...
function msgpackLite( test )
{
  ...
  var serialize = _.gdf.selectContext({ inFormat : 'structure', outFormat : 'buffer.node', ext : 'msgpack.lite' });
  test.identical( serialize.length, 1 );
  serialize = serialize[ 0 ];

  var deserialize = _.gdf.selectContext({ inFormat : 'buffer.node', outFormat : 'structure', ext : 'msgpack.lite' });
